...
import type { User, Evaluation, Feedback, Attendance, MonthlyAttendance, ReportData, DailyAttendance, EmployeeSchedule } from '@/types/firestore';
import { getMonth, getYear, getDaysInMonth, startOfMonth, endOfMonth, getDay } from 'date-fns';

/**
 * Generates the monthly attendance data for a given employee and month.
 * @param employeeId - The ID of the employee.
 * @param employeeSchedule - The work schedule of the employee.
 * @param date - A date within the desired month.
 * @returns A MonthlyAttendance object.
 */
export async function getMonthlyAttendanceData(employeeId: string, employeeSchedule: EmployeeSchedule, date: Date): Promise<MonthlyAttendance> {
    ...
    for (let i = 1; i <= daysInMonth; i++) {
        ...
        if (!isWorkDay) {
            ...
            else if (employeeSchedule === 'full-day') {
                if (morningPresent && afternoonPresent) status = 'present';
                else if (morningCheckIn || morningCheckOut || afternoonCheckIn || afternoonCheckOut) status = 'incomplete';
            } else if (employeeSchedule === 'morning') {
                if (morningPresent) status = 'present_morning';
                else if (morningCheckIn || morningCheckOut) status = 'incomplete';
            } else if (employeeSchedule === 'afternoon') {
...
export async function generateEmployeeReportData(employeeId: string, date: Date): Promise<ReportData | null> {
  ...
  const monthlyAttendance = await getMonthlyAttendanceData(employeeId, employee.schedule || 'full-day', date);
  const attendanceRecords = Object.values(monthlyAttendance);
  
  const tardinessCount = attendanceRecords.filter(r => r.status === 'late').length;
