...
export function FollowUsPageContent() {
  const [loading, setLoading] = useState(false); // No longer loading from DB

  const instagramPosts = samplePosts.filter(p => p.platform === 'Instagram');
  const tiktokPosts = samplePosts.filter(p => p.platform === 'TikTok');
  const facebookPosts = samplePosts.filter(p => p.platform === 'Facebook');
  
  // useEffect to load the Instagram script
  React.useEffect(() => {
    ...
    // Optional: Render embeds after script is loaded
  ...
  const renderContent = (platformPosts: typeof samplePosts) => {
      if (loading) {
          return (
