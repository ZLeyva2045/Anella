...
const LIBRARIES: ('places' | 'geometry' | 'marker')[] = ['geometry', 'places'];
...
export function DeliveryMap({ onLocationSelect }: DeliveryMapProps) {
  ...
  const renderMap = () => {
    if (loadError) return <p className="text-center text-destructive">Error al cargar el mapa. Verifica la API Key y que las APIs estén habilitadas.</p>;
